...
final class SheetPresentationController: UIPresentationController {
  ...
  private lazy var visualEffect: UIColor = {
  ...
  private let handleView: UIView = {
    let view = UIView()
      view.backgroundColor = .lee
    view.frame.size = CGSize(width: 50, height: 4)
    return view
  }()
  ...
  override func presentationTransitionWillBegin() {
    ...
    presentedViewController.transitionCoordinator?.animate(alongsideTransition: { [weak self] _ in
      ...
      self.backgroundView.backgroundColor = self.visualEffect
    })
  }
